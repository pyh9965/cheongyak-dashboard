...
export function useAptData() {
  ...
  const fetchSingleExtraData = useCallback(async (houseManageNo: string, pblancNo: string) => {
    ...
    try {
      ...
      if (USE_SQLITE) {
        // SQLite 모드: /api/apt/detail/{key} 호출
        const res = await fetch(`/api/apt/detail/${houseManageNo}_${pblancNo}`);
        if (!res.ok) throw new Error("API failed");

        const json = await res.json();
        const result = buildApplicationRows(
          (json.models || []) as NoticeModelApiRow[],
          (json.competition || []) as NoticeCompetitionApiRow[],
          (json.special || []) as NoticeSpecialApiRow[]
        );
        ...
        if (result.totals?.stages?.rank1?.request !== undefined &&
          ...
          console.log(`🌐 [SQLite API] Success: ${key}, 1순위 접수: ${result.totals.stages.rank1.request}명`);
          setExtraData(prev => ({ ...prev, [key]: result }));
        } else if (result.totals) {
          console.log(`⚠️ [SQLite API] Incomplete data: ${key}`);
          setExtraData(prev => ({ ...prev, [key]: result }));
        }
      } else {
        // JSON 캐시 모드: 기존 /api/cheongyak 호출
  ...
  useEffect(() => {
    if (USE_SQLITE) {
      // SQLite 모드: 정적 캐시 로드 생략
      setArchiveCache(null);
  ...
  const handleSearch = async (page: number = 1, overrideParams?: typeof searchParams) => {
    ...
    try {
      if (USE_SQLITE) {
        // SQLite 모드: /api/apt/list 호출
        const params = new URLSearchParams();
        params.set("page", "1");
        params.set("pageSize", "500");

        if (paramsToUse.startMonth) {
          params.set("startMonth", paramsToUse.startMonth.replace(/-/g, ''));
        }
        if (paramsToUse.endMonth) {
          params.set("endMonth", paramsToUse.endMonth.replace(/-/g, ''));
        }
        if (paramsToUse.sidoCode) params.set("sidoCode", paramsToUse.sidoCode);
        if (paramsToUse.sigungu) params.set("sigungu", paramsToUse.sigungu);
        ...
        if (paramsToUse.saleType) params.set("saleType", paramsToUse.saleType);

        const res = await fetch(`/api/apt/list?${params.toString()}`);
        if (!res.ok) throw new Error("API 요청 실패");

        const json = await res.json();
        let items: AptInfo[] = (json.items || []) as AptInfo[];

        // rentNo 필터링 (RENT_SECD=2)은 API에서 미지원, 클라이언트에서 처리
        if (paramsToUse.saleType === "rentNo") {
          items = items.filter((item: AptInfo) => {
            const rentSecd = item.RENT_SECD !== undefined ? String(item.RENT_SECD) : "0";
            return rentSecd === "2";
          });
        }

        console.log(`✅ [SQLite] 최종 결과: ${items.length}건`);
        setData(items);
      } else {
        // JSON 캐시 모드: 아카이브 캐시 우선, API는 fallback
        // 캐시가 로드되어 있으면 라이브 API 호출을 스킵 (Vercel 등 API 키 문제 방지)
        let mergedData: AptInfo[];

        if (archiveCache?.lists) {
          // 캐시에서 직접 로드 (API 호출 없음)
          mergedData = mergeCacheAndApiData(
            archiveCache,
            [], // API 데이터 없음 — 캐시만 사용
          ...
          const apiData = Array.from(uniqueMap.values());
          mergedData = mergeCacheAndApiData(
            archiveCache,
            apiData,
            paramsToUse.startMonth,
          ...
          console.log(`📦 [useAptData] API 데이터 병합 결과: API ${apiData.length}건, 병합후 ${mergedData.length}건`);
        }
        ...
        console.log(`✅ [useAptData] 최종 결과: ${mergedData.length}건 (${paramsToUse.startMonth} ~ ${paramsToUse.endMonth})`);
        setData(mergedData);
      }
    } catch (err) {
